...
function SessionsTable() {
  ...
  useEffect(() => { load(page, search, sortBy, sortDir); }, [load, page, search, sortBy, sortDir]);
  ...
  return (
    <div style={S.header}>
      <div style={S.headerInner}>
        ...
        {onLogout && <button style={S.logoutBtn} onClick={onLogout}>Logout</button>}
      </div>
    </div>
  );
}
...
function Pagination({ page, totalPages, onPage }) {
  ...
  return (
    <div style={S.pagination}>
      ...
      {pageNums().map((item, i) =>
        item === "…" ? (
          <span key={`gap-${i}`} style={S.pageGap}>…</span>
        ) : (
          <button
            key={item}
...
function SortTh({ col, sortBy, sortDir, onSort, children }) {
  ...
  return (
    <th
      style={{ ...S.th, cursor: "pointer", userSelect: "none", color: active ? "#1877f2" : "#8d949e" }}
      onClick={() => onSort(col)}
    >
...
const S = {
  ...
  headerInner: { maxWidth: 1200, margin: "0 auto", padding: "0 24px", height: 48, display: "flex", alignItems: "center", justifyContent: "space-between" },
  ...
  breadcrumb: { display: "flex", alignItems: "center", gap: 4, padding: "14px 0 10px", flexWrap: "wrap" },
  ...
  errorBox: { background: "#fff0f0", border: "1px solid #fa383e40", borderRadius: 8, padding: "12px 16px", color: "#fa383e", marginBottom: 16, fontSize: 13 },
  tableWrap: { background: "#fff", border: "1px solid #ccd0d5", borderRadius: 8, overflow: "hidden" },
  ...
  searchInput: { background: "#f5f6f7", border: "1px solid #ccd0d5", borderRadius: 6, padding: "6px 10px", fontSize: 12, fontFamily: "inherit", outline: "none", width: 260 },
